...
MIC_ELECTIVES: list[str] = [
    "MIC201", "MIC318", "MIC404", "MIC311", "MIC309", "MIC416", "MIC417", "MIC418"
]

# ── Prerequisite Maps ───────────────────────────────────────────────────────
# Structure: dict[course_code, list[frozenset[str]]]
# Each frozenset is an OR-group — at least ONE option must be passed.
...
def compute_total_valid_credits(
    ...
    """Credit tally with prerequisite enforcement."""
    by_course: dict[str, list[dict]] = {}
    for r in rows:
        by_course.setdefault(r["course_code"], []).append(r)
...
def resolve_mic_aliases(rows: list[dict]) -> dict[str, str]:
    ...
    for code_a, code_b in MIC_ALIAS_PAIRS:
        ...
        if pass_a and pass_b:
            ...
            # A passed, B only has F/W/I — exclude B so failures don't pollute CGPA
            exclusions[b] = a
